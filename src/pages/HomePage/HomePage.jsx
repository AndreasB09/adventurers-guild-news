...
import QuestFeed from "../../components/QuestFeed/QuestFeed";
...
const HomePage = () => {
  ...
  const filteredArticles = filterTag
    ? articles.filter(
        (article) => article.tags.includes(filterTag) && !article.featured
      )
  ...
  return (
    <div className={styles.container}>
      ...
      <p>
        Stay up to date with the latest quests, guild news, and tips for
      ...
      <div className={styles.filteredArticles}>
        {filteredArticles.map((article) => (
          <div key={article.id} className={styles.articlesContainer}>
            {article.image && (
              <img
                src={article.image}
                alt={article.title}
            ...
            <h3>{article.title}</h3>
            <p>{article.snippet}</p>
            <div>{article.tags && <span>{article.tags[0]}</span>}</div>
